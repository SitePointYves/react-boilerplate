...
export const mitarbeiterSlice = createSlice({
  ...
  reducers: {
    ...
    getMitarbeiterSuccessAction: (
      state: MitarbeiterStateType,
      { payload: mitarbeiter }: PayloadAction<MitarbeiterType>,
    ) => {
      state.mitarbeiter.isLoading = false;
      state.mitarbeiter.data = mitarbeiter;
    },
    getMitarbeiterErrorAction: (
      state: MitarbeiterStateType,
...
export const { getMitarbeiterAction, getMitarbeiterSuccessAction, getMitarbeiterErrorAction } =
  mitarbeiterSlice.actions;
